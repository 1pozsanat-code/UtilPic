...
const StartScreen: React.FC<StartScreenProps> = ({ onFileSelect }) => {
  ...
  return (
    <div 
      className={`w-full max-w-5xl mx-auto text-center p-8 transition-all duration-300 ease-in-out rounded-2xl border-2 relative ${isDraggingOver ? 'bg-blue-500/20 border-dashed border-blue-300 scale-105' : 'border-transparent'}`}
    ...
    >
      <div className={`flex flex-col items-center gap-6 animate-fade-in transition-opacity duration-300 ${isDraggingOver ? 'opacity-20' : 'opacity-100'}`}>
        ...
        <div className="mt-6 flex flex-col items-center gap-4">
            <label htmlFor="image-upload-start" className="relative inline-flex items-center justify-center px-10 py-5 text-xl font-bold text-white bg-blue-600 rounded-full cursor-pointer group hover:bg-blue-500 transition-all duration-200 active:scale-95 animate-pulse-shadow">
      ...
      {isDraggingOver && (
        <div className="absolute inset-0 flex flex-col items-center justify-center pointer-events-none animate-fade-in">
            <UploadIcon className="w-20 h-20 text-blue-300" />
            <h2 className="text-3xl font-bold text-gray-100 mt-4">Drop your image here</h2>
        </div>
      )}
    </div>
  );
